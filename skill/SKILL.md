---
name: sv-card
description: StreetVoice 街聲名片自動化製作（TW 街聲版 + 中子 BVI 版 + 台灣中子版，v0.10.0+）。觸發詞（v0.16.1 精簡為 4 個）：「做名片」（核心，無指定版型 → 依簽呈「名片版型」欄位自動判斷）/「做 SV 名片」（指定 TW 街聲版）/「做中子名片」（指定中子 BVI 版）/「做台灣中子名片」（指定台灣中子版），並附簽呈 PDF（拖入或路徑）。版型路由：「做 SV 名片」→ 期望 TW 街聲版；「做中子名片」→ 期望中子 BVI 版；「做台灣中子名片」→ 期望台灣中子版；「做名片」→ 無指定，依簽呈「名片版型」欄位判斷（「TW 街聲」→ TW 全流程；「中子BVI」/「台灣中子」→ 中子分支，跳過 vCard / QR / 上傳）。觸發語明示版型時仍與簽呈版型交叉檢核，不一致即停下問。CN / EN 版尚未支援。目前所有支援版型（TW 街聲、中子 BVI、台灣中子）皆已納入自動化白名單（v0.14.0），全自動同 TW、僅 Step 6 GATE 需確認。**未來新增版型**仍依 SOP「🆕 新版型測試 → 畢業規則」節：初期每步驟先停下確認，成功跑 ≥ 2 次後才討論加入白名單。
---
