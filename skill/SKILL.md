...
>
> **⚠️ 時機關鍵**：請在「送出 mcp call **之前**」（也就是 Claude Code UI 即將顯示 `Called illustrator` 之前）就先用文字提示使用者：**請點一下 Illustrator 以便繼續**。如果放在 mcp call 之後印，使用者在等 throttle 解除的這段時間看不到提示，會以為流程卡住。
...
```
> init 內部推導：`mobile-display`（空格→dash）、`vcf-name`（英文名去空格+.vcf）、`PH_PHONE_OFFICE`（+886-2-2741-7065#分機）。資料寫入 `/tmp/sv_card_fields.json` sidecar，Step 2/3 自動讀取。
>
> 腳本印出 `BASENAME=...` 和 `DEST_DIR=...` 給 Step 8 收尾用。
