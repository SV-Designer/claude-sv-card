...
- **專屬模板**：`SV_TEMPLATE_ZHONGZI_TAIWAN`（`templates/20260612-王小明_台灣中子.ai`），設計含 FAX 行（靜態）、僅台北一址。
...
模板位置：`~/.claude/skills/sv-card/templates/20260612-王小明.ai`（install.sh 會 symlink 進去）
...
> **v0.15.x 電話框改版**：公司電話 `+886-2-2741-7065` 改為**靜態文字框**（名為「公司電話」，**無 `PH_` 前綴 → 腳本不替換**）；分機獨立成新框 `PH_PHONE_EXT`（值 = `#`+分機，對照 PDF 室內分機；簽呈留白則框留空）。三個新版 template（TW 有手機 / 中子BVI / 台灣中子）皆此設計。
>
> **例外**：舊無手機版 `20260529-王小明_無手機版.ai` 暫未更新，仍是合成框 `PH_PHONE_OFFICE`（公司電話+分機），由 `card_helper.sh` 的 `legacy_office=1` 分支維持舊邏輯。
...
| 簽呈情況 | init 參數 | 結果 |
|---|---|---|
| 有手機 + 有分機 | `--mobile "..." --office-ext "..."` | 用預設新版模板 (`20260612-王小明.ai`)，`PH_PHONE_EXT` = `#`+分機，`PH_PHONE_MOBILE` 有值 |
| 有手機 + 無分機 | `--mobile "..." --office-ext ""` | 用預設新版模板，`PH_PHONE_EXT` 留空字串，`PH_PHONE_MOBILE` 有值 |
| 無手機 + 有分機 | `--mobile "" --office-ext "..."` | 用無手機版模板 (`20260529-王小明_無手機版.ai`，`legacy_office=1`)，舊框 `PH_PHONE_OFFICE` 含 `#`，sidecar 跳過 `PH_PHONE_MOBILE` |
| 無手機 + 無分機 | `--mobile "" --office-ext ""` | 用無手機版模板，舊框 `PH_PHONE_OFFICE` 純號碼，sidecar 跳過 `PH_PHONE_MOBILE` |

> ⚠️ **無手機版尚未更新成新電話框設計**：有手機三版已改「公司電話靜態 + `PH_PHONE_EXT`」，無手機版（20260529）暫留舊 `PH_PHONE_OFFICE` 合成框，由 `legacy_office=1` 維持相容。兩套並存，待無手機版出新版後可統一移除 legacy 分支。
...
2. `cp -L $SV_TEMPLATE` 到該資料夾並重新命名為 `{YYYYMMDD}-{chinese}_{english}.ai`（模板預設 `~/.claude/skills/sv-card/templates/20260612-王小明.ai`）
...
| 用途 | 路徑 |
|---|---|
| 模板 .ai（已含 `PH_QRCODE` 命名）| `~/.claude/skills/sv-card/templates/20260612-王小明.ai` |
