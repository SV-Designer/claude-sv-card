...
### Notes
- **兩版無手機版模板實跑驗證通過**（清除 v0.24.0 標記的「PH_* 欄位命名待實跑驗證」風險）：以假資料（王小明 / Ming Wang / 設計師 / `@neuin.com` / 分機 250 / 無手機）分別跑「台灣中子無手機版」與「中子BVI無手機版」全流程（init → replace_fields.jsx）：
  - 台灣中子無手機版：自動選對模板、`replaced=6` 零 missing、無殘留手機框。
  - 中子BVI無手機版：自動選對模板、`replaced=7`（含動態 `PH_COMPANY=中子創新有限公司`）零 missing、無殘留手機框。
- 已知邊界（暫不修，僅記錄）：測試用同名假資料時，`init` 文件就緒輪詢只比對檔名、無法區分不同資料夾的同名檔，需另查 `fullName` 完整路徑確認 active document。實務上真實名片各人姓名不同不會撞名，非問題；僅「同名不同版型批次測試」情境需注意。

## [0.24.1] — 2026-07-15

### Fixed
- **v0.24.0 新增的兩份無手機版模板漏了去識別化**：`%%For` 中繼資料欄殘留真實 macOS 帳號名（Illustrator 存檔自動寫入），與 repo 既有慣例（v0.23.x 前已將既有 4 模板 `%%For` 統一改為 `owner`）不一致。此版對兩份新模板做二進位安全取代（`owner`→`owner`，皆 5 bytes，檔案長度與 PDF 結構不變），補齊一致性。
...
### Fixed
- **`template_unsupported` 誤判「經典復刻款 BVI」為未支援版型**（`extract_signoff_fields.py`）：v0.23.0 新增此旗標時白名單只寫了 `TW街聲`／`中子BVI`／`台灣中子` 三種，漏了 `docs/SOP.md` 已記錄「輕度畢業」的第四支援版型「經典復刻款 BVI」（`card_helper.sh --template-type classic-bvi`，走手動逐步 SOP，非 card_helper 全自動）。實測用一份「經典復刻款 BVI」簽呈跑 `extract-pdf`，被誤印「⚠️ 版型非三支援版…未支援版型不自行製作」——但此版型明明是合法支援分支，訊息會誤導使用者以為完全做不了。白名單補上「經典復刻款BVI」（去空格比對），警告訊息同步改「四支援版」。
- `docs/pdf-extract.md` 同步標注四支援版與此次修正。

### Notes
- 動機：使用者提供一份「經典復刻款 BVI」簽呈實測 v0.23.0 偵測旗標，發現此誤判。真正未支援版型（CN/EN 等）測試確認仍正確觸發 ⚠️。
- 同時釐清：舊版「中子系列 PDF 中文圖片化、機械萃取全 null」的情境（`docs/pdf-extract.md`「Claude 必看項」第一條），經實測本次「經典復刻款 BVI」簽呈是**真文字層、機械萃取零 null**，與圖片化情境無關——該情境仍可能發生在特定舊版中子 PDF 產生流程，但非所有中子系列簽呈的常態。
